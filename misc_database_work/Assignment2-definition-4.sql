...
-- https://stackoverflow.com/questions/30063239/using-sql-sub-queries-in-an-insert-statement
-- cid - reference to first_name: Sara last_name: Smith
-- name - Diamond
-- notes - Should be done by Jan 2017
INSERT INTO project (cid, name, notes) 
VALUES((SELECT id FROM client WHERE first_name = "Sara" && last_name = "Smith"), "Diamond", "Should be done by Jan 2017");

-- cid - reference to first_name: David last_name: Atkins
-- name - Eclipse
-- notes - NULL
INSERT INTO project (cid, name) 
VALUES((SELECT id FROM client WHERE first_name = "David" && last_name = "Atkins"), "Eclipse");

-- cid - reference to first_name: Daniel last_name: Jensen
-- name - Moon 
-- notes - NULL
INSERT INTO project (cid, name) 
VALUES((SELECT id FROM client WHERE first_name = "Daniel" && last_name = "Jensen"), "Moon");
...
VALUES( (SELECT id FROM employee WHERE first_name = "Adam" && last_name = "Lowd"),
(SELECT id FROM project WHERE name = "Diamond"), "2012-01-01");
...
-- start_date: 8/8/2013
INSERT INTO works_on (eid ,pid ,start_date) 
VALUES( (SELECT id FROM employee WHERE first_name = "Michael" && last_name = "Fern"),
(SELECT id FROM project WHERE name = "Eclipse"), "2013-08-08");
...
INSERT INTO works_on (eid ,pid ,start_date) 
VALUES( (SELECT id FROM employee WHERE first_name = "Michael" && last_name = "Fern"),
(SELECT id FROM project WHERE name = "Moon"), "2014-09-11");
